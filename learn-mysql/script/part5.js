...
from user left outer join role on user.roleId = role.id`;
...
*/
const PART5_2_5 = `select title from content where userId = 
(select id from user where name = 'jisungPark')`;

/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-6. JiSungPark이 작성한 content의 category name을 찾기위한 SQL을 작성해주세요.
*/
const PART5_2_6 = `select category.name from category join content_category
on category.id = content_category.categoryId join content 
on content_category.contentId = content.id join user 
on content.userId = user.id where user.name = 'jiSungPark'`;
...
from content join user on content.userId = user.id where user.name = 'duRiCha'`;

/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-10. 각 user(컬럼명: name)가 작성한 글의 개수 (컬럼명: ContentCount)를 출력하기 위한 SQL을 작성해주세요.
*/
const PART5_2_10 = `select user.name as name, count(content.userId) as ContentCount from content right outer 
